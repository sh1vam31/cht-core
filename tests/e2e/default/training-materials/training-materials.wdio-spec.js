  ...
  describe('User with contact', () => {
    ...
    before(async () => {
      ...
      const firstTraining = {
        ...
        type: 'form',
        context: { start_date: new Date().getTime() - 60000, user_roles: [ 'pharmacist' ], duration: 5 },
        _attachments: {
    ...
    it('should revisit completed trainings and load uncompleted trainings', async () => {
      ...
      const secondIntroCard = await trainingCardsPage.getCardContent(
        SECOND_TRAINING_NAME,
        'intro/intro_note_1:label"]'
      );
